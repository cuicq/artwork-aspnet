...
namespace Artworks.CodeRule.Generators
{
    ...
    /// 字母种子编号生成器解释器。
    /// </summary>
    public sealed class LetterSeedCodeGeneratorInterceptor : ICodeRuleInterceptor
    {
        private static readonly Regex regex = new Regex(@"^<字母种子:(?<键格式>.*?)(:(?<宽度>\d*?))?>$");
        private readonly ISeedStore _seedStore;

        /// <summary>
        /// 构造方法。
        /// </summary>
        public LetterSeedCodeGeneratorInterceptor(ISeedStore seedStore)
        {
        ...
        public ICodeGenerator Intercept(string codeRule)
        {
            ...
            return new LetterSeedCodeGenerator(_seedStore, seedKey, seedWidth);
        }
    }
}
